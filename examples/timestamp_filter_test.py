...
print(
    f"   Oldest trade: {trades_df.select(pl.col('timestamp').min()).item().strftime('%H:%M:%S.%f')}"
)
print(
    f"   Newest trade: {trades_df.select(pl.col('timestamp').max()).item().strftime('%H:%M:%S.%f')}"
)
...
if len(monitoring_trades) == 3:
    ...
    for row in trades_df.iter_rows():
        ...
        print(
            f"   {is_after} {timestamp.strftime('%H:%M:%S.%f')}: {side} {volume} @ {price}"
        )
